	...
	public function create_loan()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($loan_id) {
				...
				if($this->input->post('terms2')=='day/s'){
					$terms = $this->input->post('terms');
					$payment = array(
						'loan_id' => $loan_id,
						'due_date' => date("Y-m-d", strtotime("+".$terms." day", strtotime($this->input->post('date_started')))),
						'due' => $this->input->post('principal'),
