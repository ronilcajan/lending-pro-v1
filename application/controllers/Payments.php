...
class Payments extends CI_Controller
{
	...
	public function reports()
	{
		...
		$data['due_date'] = $this->paymentModel->due_date();
	...
	public function save_payment()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$loan = $this->paymentModel->getPaymentdetails($payment_id);
			$prev_pment = $this->paymentModel->getPaymentdetails($payment_id-1); // get the previous payment for balance

			$to_pay = round($loan->due + $loan->p_interest + $loan->p_penalty,2);
				...
				if ($skip == 'Skip') {
					...
					if ($amount == $to_pay) {
						...
						$data = array(
							...
							'status' => 'Paid',
							'amount' => $amount,
							'balance' => empty($prev_pment->balance) ? ($loan->total_amount) - ($amount - $loan->p_penalty) : ($prev_pment->balance) - ($amount - $loan->p_penalty),
							'date' => $date,
						);

						$this->paymentModel->updatePayment($data, $payment_id);

						if($terms2 == 'day/s'){ //check if terms2 is days
							$loan_d = array(
								'status' => 'Paid',
							);
						...
						}elseif($terms != $loan->terms){// check if this payment is the last to pay
	...
	{
		$data['loan'] = $this->paymentModel->getPaymentdetails($id);
